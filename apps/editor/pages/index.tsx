...
import { destroy } from "../lib/content-api";
...
const PostList = ({ postStatus }: { postStatus: PostStatus | null }) => {
  const { isLoading, posts, error, refetch } = useFetchPostsAsAdmin(postStatus);
  ...
  if (error) {
    return (
      <p className="text-center bg-red-100 py-2 text-red-700">
        記事一覧の取得に失敗しました。`site` 側も起動してください。
      </p>
    );
  }
